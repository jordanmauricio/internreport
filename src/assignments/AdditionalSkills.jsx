...
import theresmore from "../videos/theresmore.jpg";
    ...
    render() {
        return (
            <div className="page--with-video-header">
                <VideoHeader image={theresmore} altText="Icons of tools and techniques" bgColor="#34495e" text="But wait, there’s more!"/>
                <div className="shrinkwrap">
                    ...
                    <p>ESLint creates a set of rules that determine what you should use, and if you don’t, it’ll throw errors preventing you to be able to build your application on the server. This resolves a lot of the problems with developer preferences.</p>

                    <br/><strong>Testing (TDD)</strong>
                    <p>Tests are code that you write, that will test your code. What this entails is that you describe what exactly you’re testing: e.g. that a component will render, that you should expect a certain value to be returned given another value etc. You then setup a step-by-step process on  what your test should do and execute in order to test your hypothesis, which is what you described. Tests are very useful when you’re developing, especially for continuous development. You can easily know if you had broken something in a different place after writing a new piece of code if you simply run your tests.</p>

                    <br/><strong>Configurations</strong>
                    <p>This is a broad term for a concept in web development where you write out in a separate file what environment you would like to develop in. What this means is that you can say, for example, that you’ll be writing ES6 in this application, so when the application is being build, it should convert your ES6 syntax to typical JavaScript. There are dozens of tools for this, most popular at the moment being Webpack. I will continue on this topic in the Pipelines section.</p>

                    <h3>Deployment</h3>

                    <br/><strong>GitLab Flow</strong>
                    <p>Continuing on from the Git section, GitLab Flow goes deeper into how you should send your code to the server, with the goal to optimize as much code as a developer can send with as little conflicts as possible, as smoothly as possible. There are various ideas and philosophy behind this, but as team we had chosen GitLab’s own recommendation for a flow. This flow breaks your application down into a “Master” tree where the main code is and what is usually running on the server, and various branches from this tree. These branches are called “Features”: the idea being that each branch is a copy of the Master, you develop on that branch a specific feature e.g. a comment system, and then you merge that back into the Master. This is the core of it, but barely scratches the surface of the overall idea of the flow, which is beyond the scope of this section. But simply put, knowing and understanding this flow really helps you get more work done with less hassle. </p>
